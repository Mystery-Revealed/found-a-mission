...
// the PDF → End Session (deletes everything from memory). Found a Mission is
// solo: everyone plays the same friar and starts as soon as they're approved — no
// pairing, and one class-wide accuracy group (spec §1, §6).
  ...
  if (phase === 'gate') {
    return (
      <div className="app teacher-app">
        ...
        <p className="muted">Found a Mission · everyone plays the same friar · TEKS 7.2C, 7.2B</p>
  ...
  return (
    <div className="app teacher-app">
      <header className="row">
        <div>
          <h1>Teacher Command Center</h1>
          <p className="muted">Found a Mission · session-only data · download the PDF before you end</p>
        </div>
      ...
      {pending.length > 0 && (
        <div className="card">
          ...
          <p className="muted">Approve a name and that student takes up the bell right away.</p>
          {pending.map((s) => (
      ...
      <div className="card">
        ...
        <div className="class-accuracy">
          <div className="trail-stat wide">
            <div className="trail-stat-name"><span aria-hidden="true">⛪</span> San Francisco de los Tejas — the whole class</div>
      ...
      <footer className="app-footer">7th Grade Texas History · Found a Mission · TEKS 7.2C, 7.2B</footer>
    </div>
  );
}
...
function downloadPdf(roster) {
  ...
  const doc = new jsPDF();
  const BRAND = [79, 107, 58]; // pine green

  doc.setFontSize(16);
  doc.text('Found a Mission — Session Report', 14, 16);
  doc.setFontSize(10);
  doc.setTextColor(90);
  ...
  autoTable(doc, {
    ...
    body: [['San Francisco de los Tejas', String(cls.count), cls.count ? `${cls.average}%` : '—']],
    styles: { fontSize: 9 },
    headStyles: { fillColor: BRAND },
  });
  ...
  doc.text('7th Grade Texas History · Found a Mission · TEKS 7.2C, 7.2B', 14, doc.internal.pageSize.getHeight() - 8);

  doc.save(`found-a-mission_${roster.joinCode}_${date}.pdf`);
}
