// MatchView.jsx — one chapter beat at a time: event card → your call → feedback.
// Solo, so it's always your turn. Alongside the choices, the Mission Panel shows
// the grounds growing (chapel → fields → acequia → herd) and the forty years
// passing — the build-status view that replaces a map (spec §1, §3).
...
const missionScore = (m) => (m ? (m.mission || 0) + (m.trust || 0) + (m.supplies || 0) : 0);
...
export default function MatchView({ state, dispatch }) {
  ...
  // preferring either would make the chip (and MissionPanel) read one chapter
  // ahead. Only feedback.stepIndex is baked into the feedback payload itself and
  // can't race — derive the chapter from it (2 steps per chapter, the same rule
  // the server's chapterOf uses). Once feedback is dismissed, eventCard/turn are
  ...
  const trustLow = (match.meters?.trust ?? 50) <= 15;
  const otherLow = Object.entries(match.meters || {}).find(([k, v]) => k !== 'trust' && v <= 15);

  return (
    <div className="match">
      <header className="match-header">
        <div className="nation-chip mission">⛪ Found a <b>Mission</b></div>
        <div className="trail-chip" title="Everyone plays the same friar">San Francisco de los Tejas</div>
      ...
      <div className="meters-row solo">
        <MetersBar meters={match.meters} meta={meta} title="Your Mission" />
      </div>

      {trustLow && !feedback && (
        <div className="banner danger" role="alert">
          🤝 Trust is nearly gone. If it reaches zero, the mission fails. Serve your neighbors.
        </div>
      )}
      {!trustLow && otherLow && !feedback && (
        <div className="banner danger" role="alert">
      ...
      <div className="match-body">
        <section className="action-panel" aria-live="polite">
          {feedback ? (
            ...
            <div className="waiting-panel"><div className="pulse-dot" aria-hidden="true" /><p>In the pines…</p></div>
          )}
        </section>
...
const VERDICT_UI = {
  right: { label: 'Well chosen', className: 'right', icon: '✓' },
...
function FeedbackPanel({ feedback, meta, matchEnded, onContinue }) {
  ...
  return (
    <div className="feedback-panel">
      ...
      {feedback.failed && (
        <div className="banner danger" role="alert">
          🤝 Trust has run out — and with it, the mission’s time in the woods.
        </div>
      )}
      <button className="btn big" onClick={onContinue}>
        {matchEnded || feedback.failed ? 'See how it ends' : 'Continue'}
      </button>
    </div>
  );
}
